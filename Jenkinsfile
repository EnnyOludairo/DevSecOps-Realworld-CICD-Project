...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential'){
                       ...
                       sh "docker tag reddit hennyholah/reddit:latest "
                       sh "docker push hennyholah/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image hennyholah/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@34.229.156.147 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.148.117.226:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#eo-devsecops-cicd-alerts', //update and provide your channel name
